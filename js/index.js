...
document.addEventListener("DOMContentLoaded", function () {
  ...
  showBtn.addEventListener("click", (event) => {
    ...
    const selectedDirection = selectedDirectionInput
    	? selectedDirectionInput.labels[0].textContent.trim()
      	: "all";

    const selectedCategoryInput = document.querySelector('input[name="category"]:checked');
    const selectedCategory = selectedCategoryInput
      ? selectedCategoryInput.labels[0].textContent.trim()
      : "all";
  ...
  function filterTrainers(selectedDirection, selectedCategory) {
    const filteredTrainers = DATA.filter((trainer) => {
      return ( (selectedDirection === "ВСІ" ||  trainer.specialization.toLowerCase() === selectedDirection.toLowerCase()) &&
       		 (selectedCategory === "ВСІ" || trainer.category.toLowerCase() === selectedCategory.toLowerCase())
);
    });
